import { defineComponent as S, openBlock as m, createElementBlock as v, withDirectives as b, createElementVNode as u, normalizeStyle as w, vShow as O, createCommentVNode as C, normalizeClass as I, toDisplayString as H } from "vue";
const W = {};
...
function $(t) {
  var e = new DataView(t), i = e.byteLength, s, r, o, h, n, p, c, a, l, f;
  ...
  methods: {
    ...
    moveImg(t) {
      ...
      s = e - this.moveX, r = i - this.moveY, this.$nextTick(() => {
        if (this.centerBox) {
          let o = this.getImgAxis(s, r, this.scale), h = this.getCropAxis(), n = this.trueHeight * this.scale, p = this.trueWidth * this.scale, c, a, l, f;
          switch (this.rotate) {
            case 1:
            case -1:
            case 3:
          ...
          o.x1 >= h.x1 && (s = c), o.y1 >= h.y1 && (r = a), o.x2 <= h.x2 && (s = l), o.y2 <= h.y2 && (r = f);
        }
    ...
    changeCropNow(t) {
      ...
      if (this.centerBox) {
        ...
        o = c.x1 > 0 ? c.x1 : 0, h = c.y1 > 0 ? c.y1 : 0, s > a && (s = a), r > l && (r = l);
      }
    ...
    calculateSize(t, e, i, s, r, o) {
      const h = t / e;
      let n = r, p = o;
      return n < i && (n = i, p = Math.ceil(n / h)), p < s && (p = s, n = Math.ceil(p * h), n < i && (n = i, p = Math.ceil(n / h))), n < r && (n = r, p = Math.ceil(n / h)), p < o && (p = o, n = Math.ceil(p * h)), { width: n, height: p };
    },
    // 创建完成
    endCrop() {
    ...
    getCropChecked(t) {
      ...
      i.onload = () => {
        if (this.cropW !== 0) {
          ...
          let f = this.cropW * l, d = this.cropH * l, g = r * this.scale * l, X = o * this.scale * l, x = (this.x - h + this.trueWidth * (1 - this.scale) / 2) * l, y = (this.y - n + this.trueHeight * (1 - this.scale) / 2) * l;
          c(f, d), a.save(), this.fillColor && (a.fillStyle = this.fillColor, a.fillRect(0, 0, e.width, e.height));
          const Y = (this.rotate || 0) * 90 * Math.PI / 180;
          this.full ? (c(f / this.scale, d / this.scale), a.translate(x / this.scale, y / this.scale), a.rotate(Y), a.drawImage(i, 0, 0, g / this.scale, X / this.scale)) : (a.translate(x, y), a.rotate(Y), a.drawImage(i, 0, 0, g, X)), a.restore();
        } else {
          ...
          if (f.save(), this.fillColor && (f.fillStyle = this.fillColor, f.fillRect(0, 0, e.width, e.height)), rotateY) {
            const d = Math.cos(rotateY * Math.PI / 180), g = a / 2 - a * d / 2;
            f.transform(d, 0, 0, 1, g, 0);
          }
          if (rotateX) {
            const d = Math.cos(rotateX * Math.PI / 180), g = a / 2 - a * d / 2;
            f.transform(1, 0, 0, d, 0, g);
          }
          switch (s) {
            case 0:
              c(a, l), f.drawImage(i, 0, 0, a, l);
              break;
            case 1:
            case -3:
              c(l, a), f.rotate(s * 90 * Math.PI / 180), f.drawImage(i, 0, -l, a, l);
              break;
            case 2:
            case -2:
              c(a, l), f.rotate(s * 90 * Math.PI / 180), f.drawImage(i, -a, -l, a, l);
              break;
            case 3:
            case -1:
              c(l, a), f.rotate(s * 90 * Math.PI / 180), f.drawImage(i, -a, 0, a, l);
              break;
            default:
              c(a, l), f.rotate(s * 90 * Math.PI / 180), f.drawImage(i, 0, 0, a, l);
          }
          f.restore();
        }
        t(e);
    ...
    showPreview() {
      ...
      s.w = t, s.h = e, s.url = this.imgs, s.img = {
        ...
        transform: `scale(${i})translate3d(${r}px, ${o}px, ${h}px)rotateZ(${this.rotate * 90}deg) perspective(1000px) rotateX(${this.rotateX}deg) rotateY(${this.rotateY}deg)`
      }, s.html = `
  ...
  mounted() {
    ...
    this.isIOS = !!e.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/), HTMLCanvasElement.prototype.toBlob || Object.defineProperty(HTMLCanvasElement.prototype, "toBlob", {
      value: function(i, s, r) {
        for (var o = atob(this.toDataURL(s, r).split(",")[1]), h = o.length, n = new Uint8Array(h), p = 0; p < h; p++)
          n[p] = o.charCodeAt(p);
  ...
  return m(), v("div", {
    ...
    b(u("div", {
      ...
      t.info ? (m(), v("span", {
        key: 0,
        class: "crop-info",
        style: w({ top: t.cropInfo.top })
      }, H(t.cropInfo.width) + " × " + H(t.cropInfo.height), 5)) : C("", !0),
      t.fixedBox ? C("", !0) : (m(), v("span", U, [
        u("span", {
...
const M = /* @__PURE__ */ N(A, [["render", F], ["__scopeId", "data-v-cf41ef32"]]), R = function(t) {
